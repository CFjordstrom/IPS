...
let rec copyConstPropFoldExp (vtable : VarTable)
    ...
    match e with
        ...
        | Index (name, e, t, pos) ->
            ...
            *)
            let e_opt = copyConstPropFoldExp vtable e
            match SymTab.lookup name vtable with
                | Some (ConstProp v)    -> failwith "can't happen" // first argument of Index has to be the name of an array
                | Some (VarProp y)      -> Index(y, e_opt, t, pos)
                | None                  -> Index(name, e_opt, t, pos)
        ...
        | Let (Dec (name, e, decpos), body, pos) ->
            ...
            match e' with
                | Var (varname, pos) ->
                    ...
                    *)
                    let new_vtable = SymTab.bind name (VarProp varname) vtable
                    let opt_body = copyConstPropFoldExp new_vtable body
                    Let (Dec (name, e', decpos), opt_body, pos)
                ...
                | Constant (constant, pos) ->
                    (* TODO project task 3:
                        Hint: I have discovered a constant-copy statement `let x = 5`.
                              ...
                              and optimize the `body` of the let.
                    *)
                    let new_vtable = SymTab.bind name (ConstProp constant) vtable
                    let opt_body = copyConstPropFoldExp new_vtable body
                    Let (Dec (name, e', decpos), opt_body, pos)
                ...
                | Let (Dec (y, e1, posy), e2, pos2) ->
                    (* TODO project task 3:
                        ...
                        `let name = (let y = e1 in e2) in body`
                        to
